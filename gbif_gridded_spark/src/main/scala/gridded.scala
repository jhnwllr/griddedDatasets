...
object gridded {
	...
	distinct().
	filter($"datasetkey" =!= "4fa7b334-ce0d-4e88-aaae-2e0c138d049e"). // remove eBird
	filter($"datasetkey" =!= "38b4c89f-584c-41bb-bd8f-cd1def33e92f"). // remove artportalen
	filter($"datasetkey" =!= "8a863029-f435-446a-821e-275f4f641165"). // remove observation.org
	filter($"datasetkey" =!= "50c9509d-22c7-4a22-a47d-8c48425ef4a7") // remove iNaturalist
	...
	val df_export_1 = df_1.withColumn("max_percent", max(col("percent")).over(windowSpec)).
	filter($"percent" === $"max_percent").
	cache()

	val df_export_2 = df_2.withColumn("max_percent", max(col("percent")).over(windowSpec)).
	filter($"percent" === $"max_percent").
	cache()

	val df_export = Seq(df_export_1,df_export_2).
	reduce(_ union _)
